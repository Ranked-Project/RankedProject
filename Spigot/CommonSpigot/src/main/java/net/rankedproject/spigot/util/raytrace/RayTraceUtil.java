...
import net.rankedproject.spigot.util.MathHelper;

@UtilityClass
public class RayTraceUtil {

    public Vector3d calculateIntercept(Area area, Vector3d pos, Vector3d ray) {
        Vector3d vec3 = RayTraceUtil.getIntermediateWithXValue(pos, ray, area.minX);
        Vector3d vec31 = RayTraceUtil.getIntermediateWithXValue(pos, ray, area.maxX);
        Vector3d vec32 = RayTraceUtil.getIntermediateWithYValue(pos, ray, area.minY);
        Vector3d vec33 = RayTraceUtil.getIntermediateWithYValue(pos, ray, area.maxY);
        Vector3d vec34 = RayTraceUtil.getIntermediateWithZValue(pos, ray, area.minZ);
        Vector3d vec35 = RayTraceUtil.getIntermediateWithZValue(pos, ray, area.maxZ);
        if(!RayTraceUtil.isVecInYZ(area, vec3)) {
            vec3 = null;
        }

        if(!RayTraceUtil.isVecInYZ(area, vec31)) {
            vec31 = null;
        }

        if(!RayTraceUtil.isVecInXZ(area, vec32)) {
            vec32 = null;
        }

        if(!RayTraceUtil.isVecInXZ(area, vec33)) {
            vec33 = null;
        }

        if(!RayTraceUtil.isVecInXY(area, vec34)) {
            vec34 = null;
        }

        if(!RayTraceUtil.isVecInXY(area, vec35)) {
            vec35 = null;
        }
